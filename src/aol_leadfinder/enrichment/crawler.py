...
_SUBPAGE_HINTS = [
    "about", "services", "service", "products", "product", "contact", "company",
    "profile", "عن", "نبذة", "من-نحن", "خدمات", "تواصل", "اتصل", "المنتجات",
]
...
def discover_subpages(home_html: str, base_url: str, limit: int = 3) -> list[str]:
    ...
    for a in soup.find_all("a", href=True):
        ...
        if any(h in haystack for h in _SUBPAGE_HINTS):
            key = parsed.path.rstrip("/")
            if key and key not in seen and full != base_url:
                seen.add(key)
                found.append(full)
        if len(found) >= limit:
            break
    ...
    for h in htmls:
        for email in extract_emails_from_html(h):
            if email not in emails:
                emails.append(email)
        phone = extract_phone_from_html(h)
